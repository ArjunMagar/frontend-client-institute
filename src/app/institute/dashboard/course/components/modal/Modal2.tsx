...
import { resetStatus, updateCourse } from "@/lib/store/course/courseSlice";
...
function Modal2({ id, closeModal2 }: ModalProps) {
    ...
    const [courseData] = courses.filter((course) => course.courseId === id)
    ...
    useEffect(() => {
        if (courseData) {
            setData((prev) => ({
                ...
                courseImageUrl: courseData.courseThumbnail
            }))
        }
    }, [courseData])
    ...
    const handleSubmit = (e: ChangeEvent<HTMLFormElement>) => {
        ...
        if (!token || !id) return
        dispatch(updateCourse(token, id, data))
    };
                    ...
                    >
                        ...
                        <div>
                            <label
                                ...
                                className="block font-semibold text-gray-700 mb-1"
                            >
                                Course Image <span className="text-gray-700/50">(Optional)</span>
                            </label>
                            <input
                                onChange={handleChange}
                                type="file"
